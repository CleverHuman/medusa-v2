...
const service = axios.create({
  ...
  // 超时 - 针对Tor网络设置为1分钟（60秒）
  // 原值10秒对Tor网络太短，5分钟对用户体验太差，1分钟是合理平衡
  timeout: 60000
})
...
service.interceptors.response.use(res => {
    ...
    if (code === 401) {
      if (!isRelogin.show) {
        isRelogin.show = true;
        MessageBox.confirm('The login status has expired. You can continue to stay on this page or log in again', 'system prompt', { confirmButtonText: 'Log in again', cancelButtonText: 'Cancel', type: 'warning' }).then(() => {
