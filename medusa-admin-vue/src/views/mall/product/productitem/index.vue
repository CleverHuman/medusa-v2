    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="ProCode" prop="productId">
          <el-input v-model="form.productId" placeholder="Please enter ProCode" disabled />
        </el-form-item>
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getProduct2(id).then(response => {
        ...
        // Ensure productId is set from route parameter, not from database
        this.form.productId = this.queryParams.productId;
        this.form.category = 'Fruit';
        this.form.status = String(this.form.status);
        // 保证unit有值
        if (!this.form.unit) this.form.unit = 'g';
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (!this.form.model) {
            this.$modal.msgError("Amount is required to generate SKU");
            return;
          }
          // Use productId from route parameter
          this.form.productId = this.queryParams.productId;
